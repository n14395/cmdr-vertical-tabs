...
- **Mouse back / forward on macOS at the driver level.** Cmdr already walks pane history from the X1/X2 side buttons
  ...
  `MouseEvent.button` from the DOM. Some macOS mouse drivers — Logi Options+, for one — post a swipe gesture and no
  mouse button at all, so nothing reaches it. This fork adds a native AppKit monitor that catches both shapes.

I'd like these to land upstream rather than live here forever; see the pull requests on
[vdavid/cmdr](https://github.com/vdavid/cmdr/pulls).

## Building

Everything else — dev setup, tooling, tests — is unchanged from upstream. See
...
- **Personal use is free** — install it on as many of your own machines as you like, as long as you're the only user.
- **Any other production use** — work, contract, or business — **requires a commercial license** from
