...
describe('volume switch (P4 — truly optimistic, synchronous commit)', () => {
    ...
    it("shifts focus to the navigated pane for a 'user' source", () => {
        navigate(
            { pane: 'right', to: { selectVolume: { volumeId: 'ext', path: '/Volumes/Ext' } }, source: 'user' },
            h.deps,
        )
    ...
    it("does NOT shift focus at commit for an 'mcp' source (the listener shifts synchronously instead)", () => {
        ...
        navigate(
            { pane: 'right', to: { selectVolume: { volumeId: 'ext', path: '/Volumes/Ext' } }, source: 'mcp' },
            h.deps,
        )
        expect(h.setFocusedPane).not.toHaveBeenCalled()
    })
...
describe('{ volumeId, path } volume-(re)select — ALWAYS the switch arm (guards the C1 regression)', () => {
    ...
    it('re-selecting Network from inside a host clears the host, so the pane lands on the host list', () => {
        ...
        navigate(
            { pane: 'left', to: { selectVolume: { volumeId: 'network', path: 'smb://' } }, source: 'user' },
            h.deps,
        )
...
describe('refusal strings (L12) — byte-for-byte contract', () => {
    ...
    it('pane-unavailable returns the exact "Pane not available" string', () => {
        ...
        expect(result).toEqual({
            status: 'refused',
            reason: { kind: 'pane-unavailable', message: 'Pane not available' },
        })
    })
})
