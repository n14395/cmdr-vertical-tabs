...
describe('query-ui interpolated-string parity (en)', () => {
    it('resolves the index-ready empty-state line with plural agreement', () => {
        ...
        expect(tString('queryUi.empty.indexReady', { countText: '1,234', count: 1234 })).toBe(
            'Index ready · 1,234 entries',
        )
    })

    it('resolves the results status-bar count line', () => {
        expect(tString('queryUi.results.resultCount', { shownText: '30', totalText: '1,234' })).toBe(
            '30 of 1,234 results',
        )
