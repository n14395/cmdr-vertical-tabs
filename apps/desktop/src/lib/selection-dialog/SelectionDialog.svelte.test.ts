...
const { translateSelectionMock, addRecentMock, getRecentMock } = vi.hoisted(() => ({
    ...
    translateSelectionMock: vi.fn(
        (...args: [string, string[], (boolean | null)?]): Promise<SelectionTranslateResult> => {
            void args
...
describe('SelectionDialog', () => {
    ...
    it('type-in-AI: passes the current type as context and applies a returned folder type', async () => {
        ...
        const folders = Array.from(
            overlay.querySelectorAll<HTMLElement>('[aria-label="Filter by type"] .tg-item'),
        ).find((el) => el.textContent.trim() === 'Folders')
