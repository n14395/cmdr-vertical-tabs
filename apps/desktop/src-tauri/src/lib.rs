...
pub fn run() {
    ...
    builder
        ...
        .setup(move |app| {
            ...
            // Watch the mouse's back / forward side buttons. macOS only: WKWebView
            // doesn't deliver them to the DOM, so AppKit is the only place they can
            // be read; on Linux the frontend reads them straight off the DOM.
            #[cfg(target_os = "macos")]
